...
export async function PATCH(req: Request) {
  ...
  const { id, status, admin_notes, user_id, course_slug } = await req.json();
  ...
  if (status === "approved" && user_id && course_slug) {
    await supabase.from("course_access").upsert({ user_id, course_slug });
  }
